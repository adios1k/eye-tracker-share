...
class AdvancedMetricsCalculator:
    ...
    def calculate_temporal_consistency(self, predictions: List[Dict], 
        ...
        prediction_stability = 1.0 / (1.0 + np.var(prediction_durations))
        ...
        temporal_coherence = np.corrcoef(pred_sequence, gt_sequence)[0, 1]
        ...
        false_oscillation_rate = false_oscillations / len(pred_sequence)
        
        # Calculate mean prediction duration
        mean_prediction_duration = np.mean(prediction_durations) if prediction_durations else 0
        
        return TemporalConsistencyMetrics(
            prediction_stability=prediction_stability,
            transition_smoothness=transition_smoothness,
            temporal_coherence=temporal_coherence,
            false_oscillation_rate=false_oscillation_rate,
            mean_prediction_duration=mean_prediction_duration
        )
    ...
    def profile_performance(self, inference_times: List[float],
        ...
        throughput_metrics = {
            ...
            'average_latency': total_time / len(inference_times) if inference_times else 0,
            'efficiency': total_frames / (total_time * np.mean(memory_usage)) if total_time > 0 and np.mean(memory_usage) > 0 else 0
        }
        
        # Calculate overall resource efficiency
        resource_efficiency = (throughput_metrics['frames_per_second'] * 
                             (1.0 / (1.0 + memory_usage_stats['mean'] / 1000)))
